...
export default class Controller extends Base {
  ...
  ready() {
    ...
    var mat = new THREE.ShaderMaterial({
      vertexShader: vs,fragmentShader: fs,uniforms: this.uniforms,transparent: true,
    ...
    tl
      ...
      .add(()=>{
        ...
        if (index==0) {
          ...
          TweenMax.to(this.prev.rot, 2.5, {x: -Math.PI/10*0.2,z: Math.PI/10*0.2,ease: Power2.easeInOut})
          TweenMax.to(this.next.rot, 2.5, {x: -Math.PI/10*0.2,z: Math.PI/10*0.2,ease: Power2.easeInOut})
        }
